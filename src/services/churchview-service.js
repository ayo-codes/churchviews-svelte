...
export const churchviewService = {
	...
	async login(email, password) {
		try {
			...
			if (response.data.success) {
				...
				localStorage.churchview = JSON.stringify({
					email: email,
					token: response.data.token,
					id: response.data.id
				});
	...
	async createChurch(id, church) {
		try {
			const response = await axios.post(
				this.baseUrl + '/api/denominations/' + id + '/churches',
				church
			);
			latestChurch.set(church);
			return response.status == 201;
		} catch (error) {
			return false;
		}
	...
	async getChurchById(id) {
		try {
			const response = await axios.get(this.baseUrl + '/api/churches/' + id);
			return response.data;
		} catch (error) {
			return [];
		}
